...
const FILE_EXPLORER_MAX_FILES: usize = 500;

/// Resolve and validate a device path against an allowed root.
/// Returns the canonical absolute path on the device.
fn resolve_device_path(
    ...
    // Reject obvious traversal before shell
    if input.split('/').any(|seg| seg == "..") {
        return Err("Path traversal is not allowed".to_string());
    }

    // Resolve symlinks and normalize on the device
    let real = run_adb_cmd_with_device(adb, serial, &["shell", "realpath", input])?;
    if real.is_empty() {
    ...
    if root.is_empty() {
        return Err("Allowed root is empty".to_string());
    }

    // Ensure real starts with allowed_root and is either equal or a child
    if !real.starts_with(root)
        || (real.len() > root.len() && !real[root.len()..].starts_with('/'))
    {
        return Err(format!(
            "Access denied: {} is outside allowed root {}",
            real, root
        ));
    }
...
fn entry_type_from_perm(perm: &str) -> FsEntryType {
    if perm.is_empty() {
        return FsEntryType::Other;
    }
    match perm.chars().next().unwrap() {
        'd' => FsEntryType::Folder,
        '-' => FsEntryType::File,
        'l' => FsEntryType::Symlink,
        _ => FsEntryType::Other,
    }
}
...
fn parse_ls_line(line: &str) -> Option<FsEntry> {
    ...
    // drwxrwx--x  2 root root 4096 2024-01-15 14:30 Projects
    // -rw-rw----  1 root root 1024 2024-01-10 09:00 readme.txt
    let trimmed = line.trim();
    ...
    let modified_epoch_secs = parse_ls_date(&date_str).unwrap_or(0);

    Some(FsEntry {
        name,
        path: String::new(), // filled later
        entry_type: entry_type_from_perm(&permissions),
        size_bytes,
        modified_epoch_secs,
        permissions,
    })
...
fn parse_ls_date(date_str: &str) -> Option<u64> {
    // Parse common Android ls formats as local time; epoch is for relative display only.
    if let Ok(dt) = chrono::NaiveDateTime::parse_from_str(date_str, "%Y-%m-%d %H:%M") {
        let local_dt = chrono::Local::now().timezone().from_local_datetime(&dt).single()?;
...
pub fn adb_list_files(
    ...
    let resolved = resolve_device_path(&adb, Some(&serial), &path, &allowed_root)?;

    let output = run_adb_cmd_with_device(
        &adb,
        Some(&serial),
        &["shell", "ls", "-la", &resolved],
    )
    .unwrap_or_default();
    ...
    for line in output.lines() {
        if let Some(mut entry) = parse_ls_line(line) {
            total_count += 1;
            if entries.len() < FILE_EXPLORER_MAX_FILES {
                entry.path = format!("{}/{}", resolved.trim_end_matches('/'), entry.name);
                entries.push(entry);
            }
        }
    }
    ...
    entries.sort_by(|a, b| {
        let a_is_folder = matches!(a.entry_type, FsEntryType::Folder);
        let b_is_folder = matches!(b.entry_type, FsEntryType::Folder);
    ...
    Ok(ListFilesResult {
        entries,
        total_count,
        truncated: total_count > FILE_EXPLORER_MAX_FILES,
    })
}
...
pub fn adb_push(
    ...
    for local in local_paths {
        let result = run_adb_cmd_with_device_timed(
            &adb,
            Some(&serial),
            &["push", &local, &device_dir],
            120,
        );
        match result {
            Ok(_) => results.push(PushResult {
...
pub fn adb_delete(
    ...
    let mut summary = DeleteSummary { deleted: 0, failed: 0 };

    for path in paths {
        let resolved = resolve_device_path(&adb, Some(&serial), &path, &allowed_root)?;
        match run_adb_cmd_with_device(&adb, Some(&serial), &["shell", "rm", "-rf", &resolved]) {
...
pub fn adb_rename(
    ...
    let resolved_old = resolve_device_path(&adb, Some(&serial), &old_path, &allowed_root)?;
    let resolved_new = resolve_device_path(&adb, Some(&serial), &new_path, &allowed_root)?;

    run_adb_cmd_with_device(
        &adb,
        Some(&serial),
        &["shell", "mv", &resolved_old, &resolved_new],
    )
    .map(|_| ())
}
...
pub fn adb_mkdir(
    ...
    let resolved_parent = resolve_device_path(&adb, Some(&serial), &parent, &allowed_root)?;
    let full_path = format!("{}/{}", resolved_parent.trim_end_matches('/'), name);
