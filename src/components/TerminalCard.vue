...
<template>
  <section class="card-glass border border-card-border rounded-2xl p-4 flex-1 min-h-0 flex flex-col">
    <div class="flex items-center justify-between mb-3 shrink-0">
      <div class="flex items-center gap-2">
        <Terminal :size="16" class="text-accent-emerald" />
        <h2 class="text-xs font-bold uppercase tracking-widest">Terminal</h2>
      </div>
    ...
    <div ref="logContainerRef" class="flex-1 overflow-y-auto mb-3 p-3 font-mono text-xs text-theme-secondary bg-black/60 rounded-t-xl border-x border-t border-theme-tertiary select-text">
      ...
      <div v-if="store.logs.length > 0" class="flex items-center gap-2">
        <div class="w-2.5 h-5 bg-gray-500 animate-pulse"></div>
      </div>
    </div>
