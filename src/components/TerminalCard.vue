...
watch(
  () => navStore.focusTerminalRequested,
  async (val) => {
    if (val) {
...
<template>
  <section class="card-glass p-4 flex-1 min-h-0 flex flex-col">
    ...
    <div
      ref="logContainerRef"
      class="flex-1 overflow-y-auto mb-3 p-3 font-mono text-xs text-theme-secondary bg-theme-terminal rounded-t-lg border-x border-t border-theme-tertiary select-text"
    >
      <p v-if="store.logs.length === 0" class="mb-1">No logs yet. Click "Auto Connect" to start.</p>
      <p
        v-for="(log, i) in store.logs"
        :key="i"
        class="mb-1 whitespace-pre-wrap break-all terminal-log-line"
      >
        {{ log }}
      </p>
