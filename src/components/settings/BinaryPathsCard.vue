...
<template>
  <section class="card-glass border border-card-border rounded-2xl p-4">
    ...
    <div class="mb-6">
      ...
      <div v-if="store.downloading === 'adb'" class="mt-3 bg-theme-btn border border-theme-secondary rounded-xl p-3">
        <div class="flex items-center justify-between mb-2">
          <span class="text-xs text-theme-secondary">⬇ Downloading ADB... {{ store.downloadProgress.percent }}%</span>
    ...
    <div>
      ...
      <div v-if="!store.scrcpyValid" class="mt-3 bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3">
        ...
        <div v-if="store.downloadInfo?.scrcpy_install_hint" class="mt-2">
          <p class="text-[10px] text-theme-muted mb-1">{{ store.downloadInfo.scrcpy_install_hint }}</p>
