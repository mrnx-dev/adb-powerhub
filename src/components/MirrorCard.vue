...
import { MonitorPlay, Settings, Square, Sun, RotateCw, AlertTriangle, Play, ChevronDown, Eye, MonitorOff, PinOff, MousePointerClick } from "lucide-vue-next";
...
<template>
  <section class="card-glass p-4">
    ...
    <!-- scrcpy Warning -->
    <div v-if="showScrcpyWarning" class="flex items-start gap-2 text-[11px] text-color-warning bg-color-warning-container border border-color-warning rounded-lg p-2.5 mb-3">
    ...
    <button @click="expanded = !expanded"
      ...
      <ChevronDown :size="12" class="transition-transform duration-200" :class="{ '-rotate-90': !expanded }" />
      Quick Actions
    </button>

    <!-- Quick Actions Grid -->
    <div v-show="expanded" class="grid grid-cols-3 gap-2">
      <button @click="store.toggleMirrorFlag('showTouches')"
        class="action-card" :class="{ 'action-card-active': store.showTouches }">
      ...
      <button @click="store.toggleMirrorFlag('turnScreenOff')"
        class="action-card"
      ...
      <button @click="store.toggleMirrorFlag('alwaysOnTop')"
        class="action-card" :class="{ 'action-card-active': store.alwaysOnTop }">
        <PinOff :size="14" />
      ...
      <button @click="store.toggleMirrorFlag('noControl')"
        class="action-card"
