  ...
  <section class="card-glass border border-card-border rounded-2xl p-4">
    ...
    <div class="flex items-center gap-2 mb-3">
      <button @click="toggleMirror" :disabled="!store.connected"
        class="bg-accent-emerald hover:bg-accent-emerald-hover text-theme-primary px-4 py-2 rounded-lg text-xs font-semibold transition-colors shadow-lg shadow-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed">
        {{ store.mirroring ? "Stop" : "Start" }}
      </button>
      <label class="flex items-center gap-2 cursor-pointer group">
        <input type="checkbox" v-model="store.recordingScreen"
          class="w-4 h-4 rounded border-theme-secondary bg-theme-input text-accent-emerald focus:ring-accent-emerald" />
        <span class="text-[10px] text-theme-secondary group-hover:text-theme-primary transition-colors">Record</span>
      </label>
    </div>
    ...
    <div class="flex gap-2">
      <button @click="store.toggleStayAwake(!store.stayAwakeEnabled)"
        ...
        :class="store.stayAwakeEnabled ? 'bg-accent-emerald/10 border border-accent-emerald/50 text-accent-emerald' : 'bg-theme-btn border border-theme-tertiary hover:border-accent-emerald/50 hover:bg-theme-hover text-theme-secondary'">
        <span class="mdi mdi-white-balance-sunny text-xs"></span>
        Stay Awake
      </button>

      <button @click="store.rotateDevice()"
        class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-theme-btn border border-theme-tertiary hover:border-accent-emerald/50 hover:bg-theme-hover transition-all text-[10px] font-medium text-theme-secondary">
