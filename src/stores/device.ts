...
export const useDeviceStore = defineStore('device', () => {
  ...
  async function connectWithPort(targetIp: string, targetPort: number) {
    ...
    try {
      ...
      try {
        await onConnected(`${targetIp}:${targetPort}`, 'manual');
      } catch (e) {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        try {
          ...
          if (attempt < maxRetries) {
            ...
            await Promise.race([new Promise((r) => setTimeout(r, RETRY_DELAY_MS)), cancelPromise]);
            if (cancelled) {
              addLog('Connection cancelled', 'info');
              toast.show('Connection cancelled', 'info');
              try {
    ...
    try {
      ...
      if (usbDevices.length > 0) {
        ...
        try {
          ...
          try {
            await invoke('adb_set_device', { deviceId: usbDevice.id });
          } catch {}
          pollFailCount = 0;
        ...
        if (deviceIp) {
          ...
          try {
            ...
            try {
              await onConnected(`${deviceIp}:5555`, 'wifi');
            } catch (e) {
              addLog(`Post-connect setup failed: ${e}`, 'error');
            }
      ...
      if (wifiDevices.length > 0) {
        ...
        try {
          await invoke('adb_set_device', { deviceId: wifiDevice.id });
        } catch {}
        pollFailCount = 0;
      ...
      if (last) {
        ...
        try {
          ...
          try {
            await onConnected(`${last.ip}:${last.port}`, last.method);
          } catch (e) {
            addLog(`Post-connect setup failed: ${e}`, 'error');
          }
          return true;
  ...
  async function pressVolUp() {
    try {
      await invoke('adb_key_vol_up');
      addLog('Volume up', 'success');
    } catch (e) {
      addLog(String(e), 'error');
    }
  }
  ...
  async function pressVolDown() {
    try {
      await invoke('adb_key_vol_down');
      addLog('Volume down', 'success');
    } catch (e) {
      addLog(String(e), 'error');
    }
  }
  ...
  async function pressMute() {
    try {
      await invoke('adb_key_mute');
      addLog('Mute toggled', 'success');
    } catch (e) {
      addLog(String(e), 'error');
    }
  }
  ...
  async function pressPower() {
    try {
      await invoke('adb_key_power');
      addLog('Power button pressed', 'success');
    } catch (e) {
      addLog(String(e), 'error');
    }
  }
  ...
  async function pressPrev() {
    try {
      await invoke('adb_key_prev');
      addLog('Previous track', 'success');
    } catch (e) {
      addLog(String(e), 'error');
    }
  }
  ...
  async function pressPlayPause() {
    try {
      await invoke('adb_key_play_pause');
      addLog('Play/Pause', 'success');
    } catch (e) {
      addLog(String(e), 'error');
    }
  }
  ...
  async function pressNext() {
    try {
      await invoke('adb_key_next');
      addLog('Next track', 'success');
    } catch (e) {
      addLog(String(e), 'error');
    }
  }
  ...
  function toggleMirrorFlag(flag: 'showTouches' | 'turnScreenOff' | 'alwaysOnTop' | 'noControl') {
    switch (flag) {
      case 'showTouches':
        showTouches.value = !showTouches.value;
        break;
      case 'turnScreenOff':
        turnScreenOff.value = !turnScreenOff.value;
        break;
      case 'alwaysOnTop':
        alwaysOnTop.value = !alwaysOnTop.value;
        break;
      case 'noControl':
        noControl.value = !noControl.value;
        break;
    }
  }
